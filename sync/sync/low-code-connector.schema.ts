...
export interface Retriever {
     ...
     * paginating requests. Request options set directly on the requester will not be ignored.
     */
...
export interface DeclarativeStream {
     ...
     */
    state_migrations?: StateMigration[];
    /**
...
export interface RecordSelector {
    ...
    schema_normalization?: SchemaNormalization;
 ...
 * Responsible for filtering records to be emitted by the Source.
 *
 * Record filter component whose behavior is derived from a custom code implementation of
 * the connector.
 *
...
export interface RecordFilter {
    $parameters?: { [key: string]: any };
    /**
     * Fully-qualified name of the class that will be implementing the custom record filter
     * strategy. The format is `source_<name>.<package>.<class_name>`.
     */
    class_name?: string;
...
export interface Authenticator {
     ...
     */
    authenticators?: { [key: string]: AuthenticatorValue };
    [property: string]: any;
}
...
export interface TokenUpdaterObject {
    ...
    /**
     * Key to Identify refresh token error in response (Refresh Token Error Status Codes and
     * Refresh Token Error Values should be also specified).
     */
    refresh_token_error_key?: string;
    /**
     * Status Codes to Identify refresh token error in response (Refresh Token Error Key and
     * Refresh Token Error Values should be also specified). Responses with one of the error
     * status code and containing an error value will be flagged as a config error
     */
    refresh_token_error_status_codes?: number[];
    /**
     * List of values to check for exception during token refresh process. Used to check if the
     * error found in the response matches the key from the Refresh Token Error Key field (e.g.
     * response={"error": "invalid_grant"}). Only responses with one of the error status code
     * and containing an error value will be flagged as a config error
     */
 ...
 *
 * Schema Loader component whose behavior is derived from a custom code implementation of
 * the connector.
 */
export interface SchemaLoader {
    ...
    file_path?: string;
    /**
     * Fully-qualified name of the class that will be implementing the custom schema loader. The
     * format is `source_<name>.<package>.<class_name>`.
     */
    class_name?: string;
    [property: string]: any;
 ...
 * low-code format. The cursor field and partition ID fields are automatically extracted
 * from the stream's DatetimebasedCursor and SubstreamPartitionRouter.
 * Example input state: { "13506132": { "last_changed": "2022-12-27T08:34:39+00:00" }
 * Example output state: { "partition": {"id": "13506132"}, "cursor": {"last_changed":
 * "2022-12-27T08:34:39+00:00"} }
 *
 * Apply a custom transformation on the input state.
 */
export interface StateMigration {
    type?:        StateMigrationType;
    $parameters?: { [key: string]: any };
    /**
     * Fully-qualified name of the class that will be implementing the custom state migration.
     * The format is `source_<name>.<package>.<class_name>`.
     */
    class_name?: string;
    [property: string]: any;
...
export interface Transformation {
    ...
    class_name?: string;
    /**
     * The predicate to filter a property by a property value. Property will be removed if it is
     * empty OR expression is evaluated to True.,
     */
    condition?: string;
    /**
